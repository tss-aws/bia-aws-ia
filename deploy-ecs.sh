...
# Script de Deploy ECS - Projeto BIA (Versão Final Corrigida)
# Autor: Amazon Q
# Versão: 3.0.0
...
DEFAULT_SERVICE="task-def-bia-ia-service-n595bd60"
...
EXAMPLES:
    # Build da imagem com commit hash
    $0 build
...
NOTES:
    - As tags são baseadas nos primeiros 8 caracteres do commit hash
    - O script automaticamente faz login no ECR
    - Task definitions são versionadas automaticamente
    - Rollback mantém a mesma configuração, apenas muda a imagem
...
# Função para criar nova task definition usando AWS CLI diretamente
create_task_definition() {
    ...
    if task_definition_exists $region $family; then
        ...
        # Obter task definition atual e criar nova versão com imagem atualizada
        local current_td=$(aws ecs describe-task-definition --region $region --task-definition $family --query 'taskDefinition' --output json)
        
        # Extrair informações necessárias da task definition atual
        local cpu=$(echo "$current_td" | jq -r '.cpu // "256"')
        local memory=$(echo "$current_td" | jq -r '.memory // "512"')
        local execution_role=$(echo "$current_td" | jq -r '.executionRoleArn')
        local network_mode=$(echo "$current_td" | jq -r '.networkMode // "bridge"')
        
        # Registrar nova task definition com imagem atualizada
        ...
        # Registrar nova task definition básica
        local result=$(aws ecs register-task-definition \
            --family "$family" \
            --network-mode bridge \
            --requires-compatibilities EC2 \
    ...
    if [ "$new_revision" != "null" ] && [ -n "$new_revision" ]; then
        ...
        log_error "Falha ao registrar task definition"
        echo "$result"
        exit 1
    fi
}
