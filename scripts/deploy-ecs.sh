...
get_current_task_definition() {
    ...
    aws ecs describe-task-definition --region $region --task-definition $family --query 'taskDefinition' --output json 2>/dev/null || {
        log_error "Task definition '$family' não encontrada"
        exit 1
    }
}
