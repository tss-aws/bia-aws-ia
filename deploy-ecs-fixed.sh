...
get_current_task_definition() {
    ...
    if task_definition_exists $region $family; then
        ...
        log_warning "Task definition '$family' não encontrada. Será criada uma nova."
        echo "null"
    fi
}

# Função para criar task definition básica
...
create_task_definition() {
    ...
    log_info "Verificando task definition atual..."
    local current_td=$(get_current_task_definition $region $family)
    
    local new_td
    if [ "$current_td" = "null" ]; then
        log_info "Criando nova task definition básica..."
        new_td=$(create_basic_task_definition $family $image_uri)
    else
        log_info "Atualizando task definition existente..."
        # Criar nova task definition com a nova imagem
        new_td=$(echo $current_td | jq --arg image "$image_uri" '
    ...
    echo "$new_td" > /tmp/task-definition.json
    
    local new_revision=$(aws ecs register-task-definition --region $region --cli-input-json "$new_td" --query 'taskDefinition.revision' --output text)
    
    if [ $? -eq 0 ]; then
        log_success "Nova task definition registrada: $family:$new_revision"
        echo "$family:$new_revision"
    else
        log_error "Falha ao registrar task definition. Verifique o arquivo /tmp/task-definition.json"
        exit 1
    fi
}
...
rollback() {
    ...
    local new_task_def=$(create_task_definition $region $family $image_uri)
