...
create_task_definition() {
    ...
    if task_definition_exists $region $family; then
        ...
        new_td=$(create_basic_task_definition $region $family $image_uri $account_id)
    fi
    ...
    local result=$(aws ecs register-task-definition --region $region --cli-input-json "$new_td" --output json)
